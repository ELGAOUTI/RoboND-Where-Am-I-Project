...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    for (int i = 0; i < img.height * img.step; i+=3) {
        if ((img.data[i] == white_pixel) and (img.data[i+1] == white_pixel) and (img.data[i+2] == white_pixel)) {
         if(((i+1) % img.step) <= (img.step/3))
             {
            left = true;
            break;
                }

	 else if(((i+1) % img.step) <= (2*(img.step/3)))
             {
            center = true;
            break;
                }
    ...
    if (left == true){drive_robot(0.0,0.1);}
    else if(center == true)  {drive_robot(0.1,0.0);}	
	else if(right == true){drive_robot(0.0,-0.1);}
